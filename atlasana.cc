...
class ATLAS_XX : public Analysis {
 public:
  ...
  void init() {
    ...
    std::vector<double> bin_pt4thjet_DIL = {25, 30, 35, 1000};  // Checked
    ...
    book_hist_emu("fourth_bjet_pt", bin_pt4thjet_DIL);
    ...
    // Event counters
    _counters["pass_emu_3b"] = bookCounter("pass_emu_3b", "pass_emu_3b");
    _counters["pass_emu_4b"] = bookCounter("pass_emu_4b", "pass_emu_4b");
    _counters["pass_ljets_3b"] = bookCounter("pass_ljets_3b", "pass_ljets_3b");
    _counters["pass_ljets_4b"] = bookCounter("pass_ljets_4b", "pass_ljets_4b");
  }
  ...
  void analyze(const Event& event) {
    ...
    if (pass_emu) {
      ...
      if (nbjets >= 3) {
        _counters["pass_emu_3b"]->fill(weight);
      }
      if (nbjets >= 4) {
        _counters["pass_emu_4b"]->fill(weight);
      }
    }

    if (pass_ljets) {
      if (nbjets >= 3 && njets >= 5) {
        _counters["pass_ljets_3b"]->fill(weight);
      }
      if (nbjets >= 4 && njets >= 6) {
        _counters["pass_ljets_4b"]->fill(weight);
      }
    }
  ...
  void finalize() {
    ...
    for (const auto& c : _counters) {
      scale(c.second, crossSection()/picobarn/sumOfWeights());
    }
  }

 private:
  std::unordered_map<std::string, Histo1DPtr> _histograms;
  std::unordered_map<std::string, CounterPtr> _counters;
